...
/// \brief Trait 用于为 publishers 特化 `declare_qos_parameters()` 函数。
struct PublisherQosParametersTraits {
  /// \brief 返回实体类型的字符串表示。
  static constexpr const char *entity_type() { return "publisher"; }
  /// \brief 返回允许的 QoS 策略列表。
  static constexpr auto allowed_policies() {
    return std::array<::rclcpp::QosPolicyKind, 9>{
        QosPolicyKind::AvoidRosNamespaceConventions,  // 避免 ROS 命名空间约定策略
        QosPolicyKind::Deadline,                      // 截止日期策略
        QosPolicyKind::Durability,                    // 持久性策略
        QosPolicyKind::History,                       // 历史策略
        QosPolicyKind::Depth,                         // 深度策略
        QosPolicyKind::Lifespan,                      // 生命周期策略
        QosPolicyKind::Liveliness,                    // 活跃度策略
        QosPolicyKind::LivelinessLeaseDuration,       // 活跃度租期时长策略
        QosPolicyKind::Reliability,                   // 可靠性策略
    };
  }
};

/// \brief Trait 用于为 subscriptions 特化 `declare_qos_parameters()` 函数。
...
/// \internal 返回将给定的 `qos` 配置文件中的 `policy` 转换为参数值。
inline ::rclcpp::ParameterValue get_default_qos_param_value(
    rclcpp::QosPolicyKind policy, const rclcpp::QoS &qos);

/// \internal 修改 `qos` 中给定的 `policy` 以使其成为 `value`。
...
inline rclcpp::ParameterValue declare_parameter_or_get(
  ...
  try {
    // 尝试声明参数。(Try to declare the parameter.)
    return parameters_interface.declare_parameter(param_name, param_value, descriptor);
  } catch (const rclcpp::exceptions::ParameterAlreadyDeclaredException &) {
    // 如果参数已经被声明，则返回已声明的参数值。
    return parameters_interface.get_parameter(param_name).get_parameter_value();
  }
}

#ifdef DOXYGEN_ONLY
/// \internal 为给定实体声明 QoS 参数。(Declare QoS parameters for the given entity.)
/**
 * \tparam NodeT 节点指针或引用类型。(Node pointer or reference type.)
 * \tparam EntityQosParametersTraits 一个具有两个静态方法的类：`entity_type()` 和
 *  `allowed_policies()`。请参阅 `PublisherQosParametersTraits` 和
 * `SubscriptionQosParametersTraits`。
 * \param options 用户提供的选项，指示是否应声明 QoS
 * 参数覆盖，哪个策略可以具有覆盖，以及可选的回调以验证配置文件。
 * \param node 将使用此节点声明参数。
 * \param topic_name 实体的主题名称。
 * \param default_qos 用户提供的 qos。它将用作声明的参数的默认值。
 * \return 基于用户提供的参数覆盖的 qos 配置文件。
 */
...
std::enable_if_t<
    (rclcpp::node_interfaces::has_node_parameters_interface<
     ...
     std::is_same<
         typename std::decay_t<NodeT>,
         rclcpp::node_interfaces::NodeParametersInterface::SharedPtr>::value),
    rclcpp::QoS>
declare_qos_parameters(
    const ::rclcpp::QosOverridingOptions
        &options,                      ///< [in] QoS 覆盖选项。(QoS overriding options.)
...
{
  // 获取节点参数接口。(Get the node parameters interface.)
  ...
  // 遍历允许的策略。(Iterate through the allowed policies.)
  for (auto policy : EntityQosParametersTraits::allowed_policies()) {
    if (std::count(options.get_policy_kinds().begin(), options.get_policy_kinds().end(), policy)) {
      ...
      // 声明参数或获取现有参数值。(Declare the parameter or get the existing value.)
      auto value = declare_parameter_or_get(
          parameters_interface,                      //
          param_name.str(),                          //
          get_default_qos_param_value(policy, qos),  //
          descriptor);

      // 应用 QoS 覆盖。(Apply the QoS override.)
      ::rclcpp::detail::apply_qos_override(policy, value, qos);
    }
  }

  // 验证回调。(Validation callback.)
  // > [!NOTE]
  // > 前面定义的时候给出的就是函数指针，这里就当做函数来用了，直接对 qos 进行判断
...
/**
 * @brief 此重载无法声明 QoS 参数，因为未提供节点参数接口。
 *
 * @tparam NodeT 节点类型 (Node type)
 * @tparam EntityQosParametersTraits QoS 参数特征 (QoS Parameters Traits)
 * @param[in] options QoS 重写选项 (QoS Overriding Options)
 * @param[in] node 节点引用 (Node reference)
 * @param[in] entity_name 实体名称 (Entity name)
 * @param[in] default_qos 默认 QoS 设置 (Default QoS settings)
 * @return rclcpp::QoS 返回默认 QoS 设置 (Return default QoS settings)
 */
template <typename NodeT, typename EntityQosParametersTraits>
std::enable_if_t<
    !(rclcpp::node_interfaces::has_node_parameters_interface<
...
declare_qos_parameters(
  ...
  // 返回默认 QoS 设置 (Return default QoS settings)
  return default_qos;
}

#endif

/// \internal 辅助函数，从字符串中获取 rmw qos 策略值。
...
// 定义一个内联函数 apply_qos_override
inline void apply_qos_override(
    rclcpp::QosPolicyKind policy,  //
    rclcpp::ParameterValue value,  //
    rclcpp::QoS &qos) {
  // 使用 switch 语句处理不同的 QoS 策略类型
  switch (policy) {
...
// 将 rmw_time_t 转换为可用作参数值的 int64_t
...
/// \internal 如果 `policy_value_stringified` 为 NULL，则抛出异常。
inline const char *check_if_stringified_policy_is_null(
...
// 定义一个内联函数 get_default_qos_param_value，获取默认的 QoS 参数值
inline ::rclcpp::ParameterValue get_default_qos_param_value(
    rclcpp::QosPolicyKind kind,  //

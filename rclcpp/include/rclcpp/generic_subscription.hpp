...
public:
   ...
   * 为了正确订阅主题，需要将此订阅添加到传递给此构造函数的节点的node_topic_interface中。
   *
   ...
   * 参考rclcpp::Node::create_generic_subscription()或rclcpp::create_generic_subscription()创建此类的实例并将其添加到node_topic_interface。
   *
   * \param node_base 指向父节点的NodeBaseInterface的指针
   * \param ts_lib 类型支持库，需要对应于topic_type
   * \param topic_name 主题名称
   * \param topic_type 主题类型
   * \param qos %QoS 设置
   * \param callback 用于序列化形式的新消息的回调
   ...
   * `%callback_group`.
   * \param options 订阅选项。
   * 目前并非所有订阅选项都受到尊重，对于此订阅唯一相关的选项是`event_callbacks`、`use_default_callbacks`、`ignore_local_publications`和`%callback_group`。
   */
  ...
  /**
   * 将消息转换为rclcpp::SerializedMessage并调用回调。
   */
  RCLCPP_PUBLIC
  void handle_message(
  ...
  /**
   * 处理将rclcpp::SerializedMessage分派给用户回调。
   */
  RCLCPP_PUBLIC
...
private:
  ...
  // 类型支持库应保持加载，因此将其存储在GenericSubscription中
